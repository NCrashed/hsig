...
честно. Осцилляторы аддитивные, фильтры на TPT/ZDF, ресемплинг на FIR с окном
Кайзера. Приоритеты в таком порядке: качество звука, эргономика комбинаторов,
скорость рендера (последняя - по остаточному принципу).
...
- стерео: панорама, `orbit` (бинауральное вращение), стерео-инструменты;
...
фрагмент с исходником: разошлись - тест падает и показывает обе версии.

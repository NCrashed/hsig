...
pulseTests =
  testGroup
    ...
    , -- Уровни это классический импульс минус его постоянная составляющая:
      -- 2-2w наверху и -2w внизу. Проверять надо именно их, иначе переход с
      -- меандра на узкий импульс тихо добавляет 5 дБ пика.
      testCase "уровни это 2-2w и -2w" $
        mapM_
          ( \w ->
              let xs = render defaultEnv (takeSec 0.2 (pulse (constant w) (constant 200)))
               in do
                    near ("верх " <> show w) 0.12 (2 - 2 * w) (U.maximum xs)
                    near ("низ " <> show w) 0.12 (negate (2 * w)) (U.minimum xs)
          )
          [0.1, 0.25, 0.5, 0.75, 0.9]

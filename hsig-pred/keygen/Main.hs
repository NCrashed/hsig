...
-- | Стационарная модель исчерпывается: я выучивал её за восемь секунд, 
-- дальше кривая ошибки ползла (docs/PRED.md, разд. про нестационарность). 
-- У стационарного процесса избыточная энтропия конечна, и трёхминутная 
-- пьеса из него невозможна не по недоработке, а потому что передавать 
-- больше нечего.
--

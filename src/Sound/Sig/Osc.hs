...
-- составляющую 2*width-1, которая ездила бы вместе с шириной.
--
-- ВНИМАНИЕ на уровни: это классический импульс, из которого вычтена его
-- постоянная составляющая, поэтому уровни не -1 и 1, а @2-2*width@ и
-- @-2*width@. Размах всегда 2, но при узкой ширине он уезжает вверх: при
-- 0.1 это 1.8 и -0.2 плюс выброс Гиббса почти до 2. Ровно -1 и 1 бывает
-- только при 0.5, где импульс совпадает с 'square'. Патч, отстроенный по
-- уровню на меандре, при переходе на узкий импульс упрётся в клип - делите
-- на @2 * max width (1 - width)@, если нужен постоянный пик.
--

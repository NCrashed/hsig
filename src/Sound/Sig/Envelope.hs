...
-- | Прямоугольное окно длиной dur со скосами по edge секунд с каждого края.
--
-- Им и задаётся длина стема: длина трека берётся из материала (разд. 8), а
-- паттерн бесконечен. Края гасятся, иначе обрыв слышен щелчком. Скос
-- зажимается половиной длины, то есть на коротком окне выходит треугольник,
-- а не ошибка.
gate :: Double -> Double -> Sig
gate edge dur = line [(0, 0), (e, 1), (dur - e, 1), (dur, 0)]
  where
    e = max 0 (min edge (dur / 2))
